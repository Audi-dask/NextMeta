...
>
> 1. `GROUP_CONCAT` 等多值拼接聚合：`mask_middle` 只遮中间 1/3，头尾明文泄露，已改为对聚合结果强制 `mask_all`；
> 2. `UNION` 合并查询：结果列由最左分支决定，已递归分析最左分支的血缘；
> 3. 子查询改名 + `SELECT *`：外层 `*` 展开子查询列，已按子查询输出列顺序传播脱敏规则。
>
> 另：查询窗口按业务要求只允许 `SELECT`/`EXPLAIN`，`WITH`（CTE）语句在查询窗口被正常拦截，不视为绕过。
...
SELECT user_account_id AS safe_col FROM orders LIMIT 1
UNION
SELECT user_account_id AS safe_col FROM orders LIMIT 1;
