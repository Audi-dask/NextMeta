...
验证
EXPLAIN UPDATE `meta_test`.test_orders SET status = 'completed' WHERE status = 'paid'; 执行计划预估影响行数
SELECT count(*) FROM `meta_test`.test_orders WHERE status = 'paid'; 查询具体影响行数
